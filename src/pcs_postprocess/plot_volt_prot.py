...
def render_case(case, fig_dir):
    ...
    V_target = safe_num(meta.get("V_target_pu"))
    if not np.isnan(V_target):
        ax.axhline(V_target, ls=":", color="gray", lw=0.8)
    ax.set_ylabel("V+ (pu)")
    ymin, ymax = ax.get_ylim()
    ax.set_ylim(min(0.0, ymin), max(1.0, ymax))
    ...
    vt = safe_num(meta.get("V_target_pu"))

    if not np.isnan(delta_t):
        suptitle = "Case %04d: %s  V_target=%.2f pu  Δt=%.4f s" % (ci, cn, vt, delta_t)
    elif np.isnan(t2):
        suptitle = "Case %04d: %s  V_target=%.2f pu  Δt=N/A (no_trip)" % (ci, cn, vt)
    else:
        suptitle = "Case %04d: %s  V_target=%.2f pu  Δt=N/A (detection_error)" % (ci, cn, vt)
